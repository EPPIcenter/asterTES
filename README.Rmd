...
head(dfgen, 3)
```
The outcome in `dfppl` is coded as 0 for successful treatment, 1 for recurrence, and 2 for loss to follow-up (censoring). 
...
Next, we estimate detection probabilities. Here, we use a simple example where the probability of detecting a strain at a locus depends on discretized parasite density:
